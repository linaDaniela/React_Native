...
const CrearEpsModal = ({ visible, onClose, onEpsCreated }) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await epsService.create(formData);
      if (response.success) {
        ...
        Alert.alert('Error', response.message || 'Error al crear EPS');
      }
    } catch (error) {
      ...
      <View style={styles.overlay}>
        <View style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>Crear EPS</Text>
