...
const CrearCitaModal = ({ visible, onClose, onCitaCreated }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        Alert.alert('Error', response.message || 'Error al crear cita');
      }
    } catch (error) {
      console.error('Error al crear cita:', error);
      Alert.alert('Error', 'Error al crear cita');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getPacienteNombre = (pacienteId) => {
    ...
    return paciente ? `${paciente.nombre} ${paciente.apellido}` : 'Seleccionar paciente';
  };

  const getMedicoNombre = (medicoId) => {
    const medico = medicos.find(m => m.id === medicoId);
    return medico ? `Dr. ${medico.nombre} ${medico.apellido}` : 'Seleccionar médico';
  };
