...
export default function MiAgendaMedico({ navigation }) {
  ...
  const loadAgenda = async () => {
    try {
      setLoading(true);
      console.log('🔍 MiAgendaMedico - Cargando agenda para médico:', user?.id);
      
      // Obtener todas las citas (el médico puede ver todas las citas)
      const response = await api.get('/citas');
      
      if (response.data.success) {
        console.log('📅 MiAgendaMedico - Citas obtenidas:', response.data.data?.length || 0);
        setCitas(response.data.data || []);
      } else {
        console.error('❌ MiAgendaMedico - Error en respuesta:', response.data.message);
        Alert.alert('Error', response.data.message || 'Error al cargar la agenda');
        setCitas([]);
      }
    } catch (error) {
      console.error('❌ MiAgendaMedico - Error al cargar agenda:', error);
      Alert.alert('Error', 'Error al cargar la agenda médica');
      setCitas([]);
    } finally {
      setLoading(false);
    }
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyStateTitle}>No hay citas en tu agenda</Text>
